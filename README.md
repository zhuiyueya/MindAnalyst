...
Mind-Analyst 是一个本地部署的内容分析与 RAG（检索增强问答）项目：支持从内容源采集作者/视频，生成转写与结构化摘要，构建向量索引，并基于作者内容进行问答与报告生成。

> 免责声明（重要）：本项目用于学习与研究。请确保你对采集/存储/处理的内容拥有合法授权或符合适用法律与平台条款；项目作者与贡献者不对用户的使用行为承担责任。
