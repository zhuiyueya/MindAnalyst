from __future__ import annotations

from fastapi import APIRouter, BackgroundTasks, Depends
from sqlalchemy.ext.asyncio import AsyncSession

from src.api.schemas.types import ContentTypeRequest
from src.database.db import get_session
from src.services.analysis_tasks import (
    run_reprocess_video_asr,
    run_resummarize_video,
)
from src.services.type_service import TypeService
from src.services.video_service import VideoService

router = APIRouter()


@router.get("/api/v1/authors/{author_id}/videos")
async def get_author_videos(author_id: str, session: AsyncSession = Depends(get_session)):
    return await VideoService(session).list_author_videos(author_id)


@router.get("/api/v1/videos/{video_id}")
async def get_video_detail(video_id: str, session: AsyncSession = Depends(get_session)):
    return await VideoService(session).get_video_detail(video_id)


@router.get("/api/v1/videos/{video_id}/playback")
async def get_video_playback_url(video_id: str, session: AsyncSession = Depends(get_session)):
    return await VideoService(session).get_playback_url(video_id)


@router.post("/api/v1/videos/{video_id}/set_type")
async def set_video_type(
    video_id: str,
    req: ContentTypeRequest,
    session: AsyncSession = Depends(get_session),
):
    return await TypeService(session).set_video_type(video_id, req.content_type)


@router.post("/api/v1/videos/{video_id}/resummarize")
async def resummarize_video(
    video_id: str,
    background_tasks: BackgroundTasks,
    session: AsyncSession = Depends(get_session),
    include_fallback: bool = False,
):
    video = await VideoService(session).get_video_detail(video_id)
    content_id = str(video["video"]["id"])
    background_tasks.add_task(run_resummarize_video, content_id, include_fallback)
    return {"status": "started", "message": "Video summarization started"}


@router.post("/api/v1/videos/{video_id}/reprocess_asr")
async def reprocess_video_asr(
    video_id: str,
    background_tasks: BackgroundTasks,
    session: AsyncSession = Depends(get_session),
):
    video = await VideoService(session).get_video_detail(video_id)
    content_id = str(video["video"]["id"])
    background_tasks.add_task(run_reprocess_video_asr, content_id)
    return {"status": "started", "message": "Transcript reprocess started"}
